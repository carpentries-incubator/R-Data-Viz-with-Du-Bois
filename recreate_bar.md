...
A few features missing are: dimensions, bar colors, background color, and title. Next, we will adapt the code to match the style, specifically: dimensions, bar width, background color, legend, and other default ```ggplot``` elements.
...
The second line tells R to use a specific Du Bois style. Examining Du Bois data portraits, we can see a style of format, color, and font text.
